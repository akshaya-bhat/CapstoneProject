...
class TLDetector(object):
    def __init__(self):
        ...
        # # The tf listener will generate warnings when looping a rosbag
    ...
    def image_cb(self, msg):
        ...
        light_wp = None
        state = None
        if not None in (self.pose, self.waypoints, self.lights):
    ...
    def process_traffic_lights(self):
        ...
        #TODO find the closest visible traffic light (if one exists) - completed
        diff_from_tl = len(self.waypoints.waypoints)
        for i, light in enumerate(self.lights):
            # Get stop line waypoint index
            stop_line = stop_line_positions[i]
            
            wp_index = self.get_closest_waypoint(stop_line[0], stop_line[1])
